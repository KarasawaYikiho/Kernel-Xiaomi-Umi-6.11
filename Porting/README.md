# Porting Docs

Planning and baseline analysis for the yefxx-based Xiaomi SM8250 kernel source tree, with `umi` kept as the current official ROM reference baseline.

## Files

| File | Description |
|------|-------------|
| `../PortingPlan.md` | Roadmap |
| `../AGENTS.md` | Persistent branch, source-role, naming, and verification policy |
| `Sm8250PortConfig.json` | Unified SM8250 workflow/source-reference/ROM baseline config |
| `BaselineLock.json` | Pinned source baseline and references |
| `Inventory.json` | Capability inventory |
| `ClassificationPhase1.md` | Phase 1 portability classification (A/B/C) |
| `GapReport.md` | Gap summary |
| `ReferenceSourceStrategy.md` | yefxx 6.11, LineageOS, liyafe, and N0Kernel source-role strategy |
| `ReferenceDriversAnalysis.md` | Donor driver analysis |
| `OfficialRomAnalysis.md` | ROM metadata |
| `OfficialRomBaseline/` | Repo-side ROM baseline metadata and compact validation images |
| `OfficialRomAlignmentFramework.md` | ROM alignment execution framework |
| `CHANGELOG.md` | Milestone history |
| `BRANCHING.md` | Branch strategy |

## Reading Order

1. `../PortingPlan.md`
2. Latest `CHANGELOG.md`
3. `ClassificationPhase1.md`
4. `GapReport.md`
5. `ReferenceSourceStrategy.md`
6. `OfficialRomAlignmentFramework.md`
