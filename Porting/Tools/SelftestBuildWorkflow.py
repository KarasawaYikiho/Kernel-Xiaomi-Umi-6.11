...
def main() -> int:
    ...
    expect_contains(text, "dtbs_rc=$rc3")
    expect_contains(text, 'make O=out ARCH=arm64 LLVM=1 LLVM_IAS=1 umi_defconfig')
    expect_contains(text, 'make O=out ARCH=arm64 LLVM=1 LLVM_IAS=1 -j"$(nproc)" Image Image.gz')
    expect_contains(text, 'make O=out ARCH=arm64 LLVM=1 LLVM_IAS=1 qcom/sm8250-xiaomi-umi.dtb')
    expect_contains(text, "if: always()")
    expect_contains(text, 'cp -v out/.config artifacts/ || true')
    expect_contains(text, 'cp -v out/arch/arm64/boot/Image artifacts/ || true')
    expect_contains(text, 'cp -v out/arch/arm64/boot/Image.gz artifacts/ || true')
    expect_contains(text, 'python3 Porting/Tools/BuildDtboImage.py')
    expect_contains(text, 'python3 Porting/Tools/BuildKonaDtboOverlays.py')
    ...
    expect_contains(phase2, 'make -C "$KERNEL_DIR" O="$OUT_DIR" ARCH=arm64 LLVM=1 LLVM_IAS=1 -j"$(nproc)" Image Image.gz')
    ...
    expect_contains(phase2, 'Porting/Tools/BuildDtboImage.py')
    expect_contains(phase2, 'Porting/Tools/BuildKonaDtboOverlays.py')
